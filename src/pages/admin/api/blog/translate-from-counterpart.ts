...
export const POST: APIRoute = async ({ request }) => {
  ...
  const sourceDescription = String(body.sourceDescription || '').trim();
  ...
  try {
    const result = await translateArticle(sourceTitle, sourceDescription, sourceMarkdown, targetLang, provider!);
